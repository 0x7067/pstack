---
name: no-comments
description: "Run the Comment Sicko review, fix accepted findings, and offer encodings for claimed constraints."
disable-model-invocation: true
---

# No comments

Run the `comment-sicko` skill in an independent read-only subagent when delegation is available. Otherwise run it directly and disclose that the review was not independent. Act on accepted findings.
...
1. Read `../pstack-harness/SKILL.md`, then give the scope to an independent read-only reviewer that reads `../comment-sicko/SKILL.md`. If delegation is unavailable, read and apply `comment-sicko` in the parent. Do not restate its rules.
