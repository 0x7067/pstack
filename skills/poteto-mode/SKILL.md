...
**Start every multi-step task with the harness's plan or todo facility. Its first item is to read the Principles section below in full.** If no tracker exists, keep the same checklist in the conversation. The principles ground every trigger here. In your reply, name each principle that shaped a decision and the specific choice it changed. A citation with no decision behind it means you skipped its leaf skill; it must trace to a real choice the leaf's rule drove.
...
- Any prose surface → the **unslop** skill. Your reply is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows the active harness's skill-authoring guidance when available.
- Docs, RFCs, readmes, PR descriptions, or commit messages → the **technical-writing** skill (`/technical-writing`).
- Before commit → inspect the diff for narrating comments, unsupported guards, dead compatibility paths, and unrelated edits. Use a dedicated cleanup skill when one is available.
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → use an available capability that drives the real surface. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
...
## Harness and subagents

Read `../pstack-harness/SKILL.md` before using plans, subagents, model overrides, questions, recurring work, transcripts, or surface-driving tools.

Tell every general subagent inside a playbook step to read the `poteto-mode` skill before working. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) own their reviewer or worker instructions; respect those instructions.

Use background execution for independent work when the harness supports it. Pass file pointers instead of inlining large context. Read the active harness's role configuration created by `setup-pstack`; otherwise inherit the parent model. Choose a fast model for mechanical work, a strong instruction-following model for precisely specified sequences, and a strong judgment model for ambiguous design, concurrency, or algorithms. Use only models confirmed available in the current harness.
...
Your first tracked actions are the matched playbook's steps, copied verbatim, before any task-specific items and before you reason about the task. The failure mode is reading a playbook then writing a bespoke plan that drops its named steps (`architect`, the throughput checkpoint). A step you choose not to do stays in the tracker with a one-line `skip: <reason>`; skipping silently is not allowed. Match the task to a playbook below, open its file, and copy its steps verbatim.
