...
6. **Verify the chain from transcripts, not self-report.** Locate each candidate's transcript through the active harness's session metadata or documented storage. Stay inside the active workspace and never glob across another project's chats. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims. If transcripts are unavailable, grade observable artifacts and name the evidence gap.
