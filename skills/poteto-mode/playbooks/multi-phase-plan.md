...
3. Explore in subagents through the adapter in `pstack-harness` (the **guard-the-context-window** principle skill). Tell each general child to read `poteto-mode`. Each returns file pointers, conventions, test commands, and entry points. No inlined dumps. If delegation is unavailable, explore the slices sequentially in the parent.
4. Copy the skeleton below into the plan file and fill every placeholder. Unless the operator names a path, write it under the repository's `docs/`. Keep every heading and every sub-block in the order shown. One section per PR. One PR is one change with its own evidence (the **sequence-verifiable-units** principle skill). Name the execution playbook in **How to read this**. Pick between `playbooks/autopilot-full.md` and `playbooks/autopilot-stack.md` per the rule at the end of `playbooks/autopilot-stack.md`. A standing program takes `playbooks/orchestrate.md`.
...
6. Run `bun <poteto-mode-skill>/scripts/check-plan.mjs <plan.md>` and fix every line it prints (the **encode-lessons-in-structure** principle skill). Resolve `<poteto-mode-skill>` from the loaded skill path. It enforces the skeleton's shape, the verification rule in every verification block, and the punctuation rules.
7. Hand back. Post the plan path and the script's output, then stop. Execution starts on the operator's explicit go, under the execution playbook the plan names.

**Verification.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked (the **prove-it-works** principle skill). That sentence is the verification rule. Every verification block opens with it. The live block is mandatory. Ten independent lanes at the PR head drive the real surface through an available driver, per the **swarm** skill. Each lane is one box with a concrete scenario, the evidence it saves, and its pass predicate. The perf block names the metric, the probe, the trunk baseline measured first, and the rule with the number that fails. A PR that changes an interaction is review-gated. The operator reviews it in chat with screenshots and a video before merge. A PR that changes no interaction writes `**Review gate.** None. <PR id> is not review-gated.` and no boxes under it.

**Surface driver.** Pick it by surface through `pstack-harness`: browser automation for browser, Electron, and web UIs; terminal interaction for CLIs and TUIs; simulator or device control for native mobile; direct APIs for services. A PR that touches two surfaces gets lanes on both. A surface with no driver is a risk in Appendix C, and its live block still names how each lane drives it.
...
- [ ] On her go, arm the harness's persistent goal when available with this exact text. Otherwise put it in the tracker and decision trail. "<The plan path, the PR ids in order, the verification rule, who merges, and the done condition.>"
...
- [ ] Arm the 30-minute audit tick through the recurring-work capability in `pstack-harness`. Never leave the cadence to memory.
- [ ] Use this tick prompt, verbatim. "Re-read the execution playbook from trunk and the persistent goal or tracked objective. Audit the operation against both and fix drift in this tick. Probe every active lane and judge progress by side effects only. Stand down a stuck lane and dispatch its replacement now. Then send the operator a status message, whether or not anything changed, with the queue table of PR, owner, state, and head SHA, the verdicts since the last tick, what merged, open operator gates, and blockers."
...
- [ ] Before each commit, remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits. Run `no-comments` before review.
...
Each live lane runs in its own isolated workspace at the PR head. Drive through the selected real-surface capability.

- [ ] `git fetch origin <head-branch> && git checkout <head SHA>`.
- [ ] <Start the backend and the surface. Wait for ready.>
- [ ] <Deliver input only through the surface driver's commands. Name the read-only diagnostics.>
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten independent lanes at the PR head, per the boot recipe.
