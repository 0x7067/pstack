...
4. Verify each component against its baseline via image diff on the matching surface through the selected surface driver. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Use the recurring-work capability in `pstack-harness` per component until the diff is zero.
