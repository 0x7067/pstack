...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation with the configured perf-issue model, or inherit the parent model; review the diff. If delegation is unavailable, implement in the parent and disclose the fallback. Capture a post-fix trace.
