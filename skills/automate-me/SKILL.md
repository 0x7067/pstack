...
This skill orchestrates an inline mining pass, the active harness's skill-authoring guidance, and the **unslop** skill. Read `../pstack-harness/SKILL.md` first.

## Flow

### 0. Check for an existing skill

Look recursively for `.agents/skills/**/*-mode/SKILL.md` and `~/.agents/skills/*-mode/SKILL.md` matching the user's handle. Include any additional skill locations exposed by the current harness. If one exists, confirm intent through a structured question when available, unless the user already said "update my skill" or similar:
...
Locate the active workspace's transcripts using session metadata or the current harness's documented storage. Stay inside that workspace. If transcripts are unavailable, use the current conversation and say so.
...
Mining misses intent that has not come up yet. Use a structured multi-choice question when the harness supports one. Otherwise ask concise plain-text questions.
...
Use the active harness's skill-authoring guidance. Placement:

- Path: preserve an existing mode skill's location. For a new project skill, use `.agents/skills/<handle>-mode/SKILL.md`. For a personal skill, use `~/.agents/skills/<handle>-mode/SKILL.md`.
...
Apply the **unslop** skill and the active skill-authoring guidance to every line. Both apply to any agent-read prose, not just skills.
...
A `-mode` skill is subjective output. A structural benchmark loop is not useful here. Vibe-check with the user: does it read like them? Did it miss anything? Then ship.

Run a description-optimization loop only if the skill's trigger accuracy turns out to be a problem in practice.

## When not to use

- User wants a task-specific skill rather than working conventions: use skill-authoring guidance alone, with no mining pass.
...
- The active harness's skill-authoring guidance: skill authoring process and validation.
