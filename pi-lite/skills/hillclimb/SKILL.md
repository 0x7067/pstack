...
5. Loop, one hypothesis per iteration:
   - Hand the change to a subagent using the configured hillclimb model, or inherit the parent model, with a tight scope; supervise and review the diff rather than typing it (the **guard-the-context-window** principle). When delegation is unavailable, implement in the parent and disclose the fallback. When several independent hypotheses are live, fan them to parallel subagents, each in its own worktree so they can't collide (the **separate-before-serializing-shared-state** principle).
