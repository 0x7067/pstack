...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) checks the diff before each commit and applies [`unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. The cleanup removes narrating comments, unsupported guards, dead compatibility paths, and unrelated edits. A dedicated cleanup skill can provide the same pass when the active harness has one.
